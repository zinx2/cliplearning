...
public class AuthService {
     ...
     * @deprecated Use {@link AuthService#requestShowAgeAuthDialog(AccountResponseCallback, Bundle, Activity)} instead.
     */
    @Deprecated
    public static Future<Integer> requestShowAgeAuthDialog(final AccountResponseCallback callback, final Bundle ageAuthParams, final boolean useSmsReceiver) {
        return KakaoTaskQueue.getInstance().addTask(new KakaoResultTask<Integer>(callback) {
            ...
            public Integer call() throws Exception {
                ...
                return AgeAuthManager.getInstance().requestShowAgeAuthDialog(ageAuthParams, activity, useSmsReceiver);
            }
        });
    }
     ...
     * (제휴를 통해 권한이 부여된 특정 앱에서만 호출이 가능합니다.)
     *
     * 앱에서 RECEIVE_SMS 권한을 획득한 상태라면 인증번호가 자동 완성됨.
     *
     * @param callback 요청 결과에 대한 callback
     * @param ageAuthParams {@link AgeAuthParamBuilder}를 통해 만든 연령인증에 필요한 파람들
     * @param activity Permission을 체크하는 용도로 사용하는 현재 앱 context
     * @return {@link AgeAuthStatus} 연령인증 요청 응답 status
     */
    public static Future<Integer> requestShowAgeAuthDialog(final AccountResponseCallback callback, final Bundle ageAuthParams, final Activity activity) {
        return KakaoTaskQueue.getInstance().addTask(new KakaoResultTask<Integer>(callback) {
            @Override
            public Integer call() throws Exception {
                boolean useSms = ContextCompat.checkSelfPermission(activity, Manifest.permission.RECEIVE_SMS) == PackageManager.PERMISSION_GRANTED;
                return AgeAuthManager.getInstance().requestShowAgeAuthDialog(ageAuthParams, activity, useSms);
            }
        });
    }
